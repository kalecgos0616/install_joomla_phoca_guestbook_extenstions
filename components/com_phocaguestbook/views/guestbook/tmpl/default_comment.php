...
$values = &$this->values;
		...
		$withTitle = $this->params->get('display_comment_name')   ||
					 $this->params->get('display_comment_email') ||
					($this->params->get('display_comment_subject') && $values->title != '') ||
					 $this->params->get('display_comment_website');	
		
		$withFoot  = $this->params->get('display_comment_date') ||
		...
		$sep = 0;
		if ($this->params->get('display_comment_name')) {
			if ($values->username != '') {
				echo phocaguestbookHelperFront::wordDelete($values->username, 40, '...');
				$sep = 1;
			}
		}
		if ($this->params->get('display_comment_email')) {
			if ($values->email != '') {
				if ($sep == 1) {
		...
		if ($this->params->get('display_comment_subject') && $values->title != '') {
			if ($sep == 1) {
				echo ': ';
			}
			echo phocaguestbookHelperFront::wordDelete($values->title, 100, '...');
		}
		if ($this->params->get('display_comment_website')) {
			if ($values->homesite != '') {
		...
		<small class="pgb_thi_font">
			<?php if ($this->params->get('display_comment_date')) {
				echo JHTML::_('date',  $values->date, JText::_( $this->params->get('date_format') ) ); 
				}
